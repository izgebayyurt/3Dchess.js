import { Chess3D, Chess, WHITE, BLACK } from '../src/chess'
...
test('attackers - return value depends on side to move', () => {
 ...
 test('attackers - every piece attacking empty square', () => {
  const chess = new Chess3D('2b5/4kp2/2r5/3q2n1/8/8/4P3/4K3/64/64/64/64/64/64/64 w - - 0 1')
 ...
 test('attackers - every piece attacking another piece', () => {
  const chess = new Chess3D('4k3/8/8/8/5Q2/5p1R/4PK2/4N2B/64/64/64/64/64/64/64 w - - 0 1')
 ...
 test('attackers - every piece defending empty square', () => {
  const chess = new Chess3D('B3k3/8/8/2K4R/3QPN2/8/8/8/64/64/64/64/64/64/64 w - - 0 1')
 ...
 test('attackers - every piece defending another piece', () => {
  const chess = new Chess3D('2r5/1b1p4/1kp1q3/4n3/8/8/8/4K3/64/64/64/64/64/64/64 b - - 0 1')
 ...
 test('attackers - pinned pieces still attack and defend', () => {
  ...
  const chess = new Chess3D(
    'r1bqkbnr/ppp2ppp/2np4/1B2p3/3PP3/5N2/PPP2PPP/RNBQK2R/64/64/64/64/64/64/64 b KQkq - 0 4',
  )
 ...
 test('attackers - king can "attack" defended piece', () => {
  const chess = new Chess3D('3k4/8/8/8/3b4/3R4/4Pq2/4K3/64/64/64/64/64/64/64 w - - 0 1')
  expect(chess.attackers('f2a', WHITE)).to.have.members(['e1a'])
})

 test('attackers - a lot of attackers', () => {
  const chess = new Chess3D(
    '5k2/8/3N1N2/2NBQQN1/3R1R2/2NPRPN1/3N1N2/4K3/64/64/64/64/64/64/64 w - - 0 1',
  )
